...
#define SOURCE_IP "192.168.31.198"
...
// #endif
    ...
    //
//     const int flagOne = 1;
//     if (setsockopt (sock, IPPROTO_IP, NULL,
// #if defined _WIN32
// 		(const char*)
// #endif
...
gettimeofday(&start,NULL);
    int lena = sizeof(dest_in);
    int rec = -1;

    while(1){
        int data_size = recvfrom(sock, packet, sizeof(packet),0,(struct sockaddr *)&dest_in,&lena);
        if (data_size > 0){
    ...
    float time_micro = (end.tv_sec - start.tv_sec)*1000.0f + (end.tv_usec - start.tv_usec);
    float time_mili = (end.tv_sec - start.tv_sec) * 1000.0f + (end.tv_usec - start.tv_usec) / 1000.0f;
    printf("RTT time in milliseconds: %f, RTT time in microseconds %f \n", time_mili, time_micro);
